...
summary: ''
authors: []
lastmod: '2022-03-02T12:47:13-04:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
ff_data_num <- as.data.frame(ff_data %>%
  mutate(month = case_when(month =='jan' ~ 1,
         ...
         day = case_when(day =='mon' ~ 1,
                         ...
                         day =='sat' ~ 6,
                         day =='sun' ~ 7)))
ff_data_num # Once again, let's see what we did.
```
...
In later posts, we'll want to predict a given feature which we refer to as our target. The remaining features will act as our inpu data to predict the target. Thus it can be usefull to split our input features and target into two different variables.
```{r}
...
In the following app, we'll practice using sliders and selection tools to filter data for description and visualization.
